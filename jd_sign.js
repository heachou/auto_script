...
let Key = ''; //该参数已废弃; 仅用于下游脚本的兼容, 请使用json串数据 ↓

let DualKey = ''; //该参数已废弃; 仅用于下游脚本的兼容, 请使用json串数据  ↓

let OtherKey = `[{
    "cookie": "pt_key=${process.env.PT_KEY};pt_pin=${process.env.PT_PIN};"
...
let stop = '0'; //自定义延迟签到, 单位毫秒. 默认分批并发无延迟; 该参数接受随机或指定延迟(例: '2000'则表示延迟2秒; '2000-5000'则表示延迟最小2秒,最大5秒内的随机延迟), 如填入延迟则切换顺序签到(耗时较长), Surge用户请注意在SurgeUI界面调整脚本超时; 注: 该参数Node.js或JSbox环境下已配置数据持久化, 留空(let stop = '')即可清除.

let DeleteCookie = false; //是否清除所有Cookie, true则开启.
...
function notify() {
  return new Promise(resolve => {
    try {
      ...
      let Cash = merge.TotalCash && merge.TotalCash.TCash ? `${merge.TotalCash.TCash}红包` : ""
      let Steel = merge.TotalSteel && merge.TotalSteel.TSteel ? `${merge.TotalSteel.TSteel}钢镚` : ``
      let beans = merge.TotalBean && merge.TotalBean.Qbear ? `${merge.TotalBean.Qbear}京豆${Steel?`, `:``}` : ""
      let Money = merge.TotalMoney && merge.TotalMoney.TMoney ? `${merge.TotalMoney.TMoney}现金${Cash?`, `:``}` : ""
      let Subsidy = merge.TotalSubsidy && merge.TotalSubsidy.TSubsidy ? `${merge.TotalSubsidy.TSubsidy}金贴${Money||Cash?", ":""}` : ""
      let Tbean = bean ? `${bean.toFixed(0)}京豆${steel?", ":""}` : ""
      let TSteel = steel ? `${steel.toFixed(2)}钢镚` : ""
      let TCash = cash ? `${cash.toFixed(2)}红包${subsidy||money?", ":""}` : ""
      let TSubsidy = subsidy ? `${subsidy.toFixed(2)}金贴${money?", ":""}` : ""
      let TMoney = money ? `${money.toFixed(2)}现金` : ""
      let Ts = success ? `成功${success}个${fail||err?`, `:``}` : ``
      let Tf = fail ? `失败${fail}个${err?`, `:``}` : ``
      let Te = err ? `错误${err}个` : ``
      let one = `【签到概览】:  ${Ts+Tf+Te}${Ts||Tf||Te?`\n`:`获取失败\n`}`
      let two = Tbean || TSteel ? `【签到奖励】:  ${Tbean+TSteel}\n` : ``
      let three = TCash || TSubsidy || TMoney ? `【其他奖励】:  ${TCash+TSubsidy+TMoney}\n` : ``
      let four = `【账号总计】:  ${beans+Steel}${beans||Steel?`\n`:`获取失败\n`}`
      let five = `【其他总计】:  ${Subsidy+Money+Cash}${Subsidy||Money||Cash?`\n`:`获取失败\n`}`
      let DName = merge.TotalBean && merge.TotalBean.nickname ? merge.TotalBean.nickname : "获取失败"
      let cnNum = ["零", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十"];
      ...
      const str = Name + one + two + three + four + five + disa + notify
      axios.post(`${fangtangUrl}?title=${encodeURI("京东签到：")}&desp=${encodeURI(str)}`)
...
function JDUserSignPre1(s, key, title, acData, ask) {
  return new Promise((resolve, reject) => {
    ...
    const JDUrl = {
      ...
      opts: {
        'filter': 'try{let od=JSON.parse(body);let params=(od.floatLayerList||[]).filter(o=>o.params&&o.params.match(/enActK/)).map(o=>o.params).pop()||(od.floorList||[]).filter(o=>o.template=="signIn"&&o.signInfos&&o.signInfos.params&&o.signInfos.params.match(/enActK/)).map(o=>o.signInfos&&o.signInfos.params).pop();let tId=(od.floorList||[]).filter(o=>o.boardParams&&o.boardParams.turnTableId).map(o=>o.boardParams.turnTableId).pop();let page=od.paginationFlrs;return JSON.stringify({qxAct:params||null,qxTid:tId||null,qxPage:page||null})}catch(e){return `=> 过滤器发生错误: ${e.message}`}'
      },
...
function disable(Val, name, way) {
  ...
  if (annal && way == 1 && boxdis) {
    let Crash = $nobyda.write("", "JD_Crash_" + Val)
    if (read) {
      if (read.indexOf(Val) == -1) {
        let Crash = $nobyda.write(`${read},${Val}`, "JD_DailyBonusDisables")
        console.log(`\n${name}-触发自动禁用 ‼️`)
      ...
      let Crash = $nobyda.write(Val, "JD_DailyBonusDisables")
      console.log(`\n${name}-触发自动禁用 ‼️`)
      merge[Val].notify = `${name}: 崩溃, 触发自动禁用 ‼️`
      merge[Val].error = 1
      $nobyda.disable = 1
    ...
    let Crash = $nobyda.write(name, "JD_Crash_" + Val)
  } else if (way == 2 && annal) {
    let Crash = $nobyda.write("", "JD_Crash_" + Val)
  }
...
function Wait(readDelay, ini) {
  ...
  if (typeof(readDelay) == 'string') {
    let readDelay = readDelay.replace(/"|＂|'|＇/g, ''); //prevent novice
